...
/*   Updated: 2021/02/19 17:46:12 by gjeon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*dot_flags(char *nb_str, t_info *info)
{
	...
	if (!(temp = (char *)malloc(sizeof(char) * (info->digit + 1))))
		return (0);
	ft_memset(temp - digit, '0', info->digit);
	ft_strlcpy(temp + (info->digit - len), nb_str, len + 1);
	ft_memset(temp, ' ', digit + minus);
	free(nb_str);
	return (temp);
}
